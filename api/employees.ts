...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const query = `INSERT INTO employees (${keys.join(',')}) VALUES (${keys.map((_,i)=>'$'+(i+1)).join(',')}) RETURNING *`;
    ...
    if (req.method === 'PUT') {
      ...
      const query = `UPDATE employees SET ${setQuery} WHERE id = ${keys.length + 1} RETURNING *`;
